...
def test_SIP_grid_position_calculation():
    assert UIUtilities.calculatePinPosition(pin_no = 1, isLabel = False, pins_per_side = [10]) == (2, 1)
    assert UIUtilities.calculatePinPosition(pin_no = 1, isLabel = True, pins_per_side = [10]) == (1, 1)
    assert UIUtilities.calculatePinPosition(pin_no = 10, isLabel = False, pins_per_side = [10]) == (2, 10)
    assert UIUtilities.calculatePinPosition(pin_no = 10, isLabel = True, pins_per_side = [10]) == (1, 10)

    with pytest.raises(Exception) as e_info:
        UIUtilities.calculatePinPosition(pin_no = 11, isLabel = True, pins_per_side = [10])

    assert e_info.value.args[0] == 'Pin structure does not fit pin number 11'
